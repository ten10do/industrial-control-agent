...
COMPLETE_PUMP_PLAN = (
    "启动条件满足后水泸运行，停止条件有效时复位输出并停止，监视运行状态和运行反馈。"
    "急停最高优先级，触发后立即停止并切断输出，人工复位；急停报警。"
    "热继电器提供过载保护，过载停止并报警。低液位检测触发防干转，缺水时停止水泵并报警。"
    "电机故障报警，反馈异常报警，液位过低报警。故障时停止，输出进入默认停止安全状态。"
)
...
_DRY_RUN_MISSING_PLAN = (
    ...
    "电机故障报警，反馈异常报警。故障时停止，输出进入默认停止安全状态。"
)

_VALVE_SCENARIO = "电动阀门，启动按钮、停止按钮、急停按钮、开到位反馈和报警灯。"
_VALVE_TIMEOUT_PLAN = (
    "启动条件满足后动作，停止条件有效时复位输出并停止，监视运行状态和开到位反馈。"
    "急停最高优先级，触发后立即停止并切断输出，人工复位；急停报警。"
    "阀门故障报警，反馈异常报警，到位超时报警。故障时停止，阀门进入安全位置。"
)
_VALVE_IO = (
    io_point("I0.0", "启动按钮", "DI", "启动按钮"),
    io_point("I0.1", "停止按钮", "DI", "停止按钮"),
...
_MULTI_CRITICAL_PLAN = (
    ...
    "监视运行状态和运行反馈。热继电器提供过载保护，过载停止并报警。"
    + ("定期检查控制回路与接线状态。" * 12)
    + "电机故障报警，反馈异常报警。"
)
