...
当前前端已重设计为工业自动化控制室 / SCADA 监控台风格，包含：
...
该界面用于展示 HTML、CSS、JavaScript 和 React 组件化开发能力，同时贴合工业自动化项目的使用场景。

## 项目亮点

1. 使用 React 组件化组织页面，拆分 Header、Sidebar、表单、状态卡片、结果 Tabs、报告预览等模块。
2. 使用 HTML / CSS / JavaScript 实现完整前端交互，不依赖复杂 UI 框架。
3. 前端采用 SCADA / 工业自动化风格，增强项目展示辨识度。
...
- `01_scada_home.png`：SCADA 风格首页、顶部状态栏和工控导航面板。
- `02_scenario_form.png`：示例场景选择和 PLC 工作台式输入表单。
...
```markdown
![SCADA 风格首页](screenshots/01_scada_home.png)
...
面向自动化控制系统初步方案设计场景，开发前后端分离的工业控制方案设计 Agent 系统。用户输入控制对象、输入设备、输出设备和控制要求后，系统通过 FastAPI 后端调用大模型，生成控制需求分析、PLC I/O 点表、控制逻辑、安全保护与报警逻辑、PLC 梯形图设计思路和 Markdown 方案报告。前端采用 SCADA / 工业自动化风格界面，支持示例场景、状态展示、结果 Tabs、I/O 表格和报告复制。

项目职责：

- 设计并实现 React + Vite 前端页面，完成 SCADA 风格 UI、表单输入、示例选择、状态卡片、结果 Tabs 和 Markdown 报告展示。
...
- 前端以 SCADA / PLC 调试平台为视觉方向，展示工程化页面设计能力，而不是普通聊天页面。
